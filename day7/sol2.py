...
def process_hand(hand):
    mappings = {"A": 14, "K": 13, "Q": 12, "J": 1, "T": 10}
    return [mappings[x] if x in mappings.keys() else int(x) for x in list(hand)]    
    

def get_ranking(hand):
    most_common_card = Counter([x for x in hand if x != 1]).most_common(1)
    if hand == [1, 1, 1, 1, 1]:
        most_common_card = [(1, 5)]
    
    hand = [x if x != 1 else most_common_card[0][0] for x in hand]
